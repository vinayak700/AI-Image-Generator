...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://encouraging-ruby-underclothes.cyclic.app/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://encouraging-ruby-underclothes.cyclic.app/api/v1/post', {
          method: 'POST',
          headers: {
            'content-type': 'application/json'
          },
  ...
  return (
    <section className='max-w-7xl mx-auto'>
      ...
      <form action="" className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        ...
        <div className='mt-10'>
          <p className='mt-2 text-[#666e75] text-[14px]'>Once You have created the image you want, you can share it with others the community</p>
